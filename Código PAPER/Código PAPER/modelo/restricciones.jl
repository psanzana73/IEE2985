...
function generar_restricciones(modelo, par, set, var; u0=nothing, p0=nothing, enforce_end::Bool=false)
    ...
    @constraint(modelo, KCL[t in set.TimeSet, b in set.BusSet],
        ...
        # NOTA: Este modelo DC asume que los IBGs no inyectan potencia activa (p_ibg)
        # en el balance nodal (solo los SGs con var.p).
    ...
    # --- Restricciones de Frecuencia y SCC ---
    
    # (Aquí iría la llamada a las restricciones de Frecuencia, 
    #  si las tuvieras definidas)
